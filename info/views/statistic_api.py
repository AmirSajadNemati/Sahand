...
from info.models import Statistic
from serializers import MessageAndIdSerializer, DeleteSerializer, ListRequestSerializer, IdSerializer
from info.serializers import StatisticSerializer, StatisticListSerializer, StatisticGetSerializer
...
class StatisticAddOrUpdateView(APIView):

    @extend_schema(
        request=StatisticSerializer,
        responses={200: OpenApiResponse(response=MessageAndIdSerializer)},
        description="Add or Update Statistic"
    )
    @role_decorator
    def post(self, request):
        statistic_id = request.data.get('id', 0)

        # Create new statistic if ID is 0
        if statistic_id == 0:
            serializer = StatisticSerializer(data=request.data)
            if serializer.is_valid():
                serializer.save()
                return Response({"message": "آمار با موفقیت ایجاد شد!", "id": serializer.instance.id},
        ...
        try:
            ...
            return Response({'message': 'آمار مورد آمار یافت نشد.', "id": statistic_id},
                            status=status.HTTP_400_BAD_REQUEST)

        serializer = StatisticSerializer(statistic, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response({"message": "آمار با موفقیت به روزرسانی شد!", "id": statistic_id},
                            status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class StatisticGetListView(APIView):

    @extend_schema(
        request=ListRequestSerializer,
        responses={200: OpenApiResponse(response=StatisticListSerializer())},
    ...
    def post(self, request):
        ...
        statistics = Statistic.objects.filter(query)

        # Sorting
        if sort:
            sort_field = sort if sort.startswith('') else f'-{sort}'
            statistics = statistics.order_by(sort_field)
        else:
            # Apply a default ordering (e.g., by 'id' or another field)
            statistics = statistics.order_by('id')  # or any field you prefer

        # Pagination
        paginator = StatisticPagination()
        paginated_statistics = paginator.paginate_queryset(statistics, request)

        # Serialize the paginated data
        serializer = StatisticSerializer(paginated_statistics, many=True)
        data = paginator.get_paginated_response(serializer.data)

        # Define the propertiesAttribute for the Statistic model
        properties_attribute = {
            "propertyName": "Statistic Text",
...
class StatisticGetView(APIView):

    @extend_schema(
        request=IdSerializer,
        responses={
            200: OpenApiResponse(response=StatisticGetSerializer),
            400: OpenApiResponse(description='Not Found')
        },
        description="Get a specific Statistic by ID"
    )
    @role_decorator
    def post(self, request):
        statistic_id = request.data.get('id', 0)
        ...
        # Prepare the response structure with manual property attribute definitions for Statistic fields
...
class StatisticDeleteView(APIView):

    @extend_schema(
        request=DeleteSerializer,  # The request body will contain the `id` of the cms
        responses={200: OpenApiResponse(response=MessageAndIdSerializer)},
        description="Delete a specific Statistic by ID"
    )
    @role_decorator
    def post(self, request):
        statistic_id = request.data.get('id')
        ...
        try:
            ...
            return Response({"message": "آمار یافت نشد!"}, status=status.HTTP_400_BAD_REQUEST)

        if delete_type == 1:
            # Soft delete: Set is_deleted to True
            if statistic.is_deleted:
                return Response({"message": "آمار از قبل حذف نرم شده است."}, status=status.HTTP_400_BAD_REQUEST)
            statistic.is_deleted = True
            statistic.save()
            return Response({"message": "آمار به صورت نرم پاک شد."}, status=status.HTTP_200_OK)
        elif delete_type == 2:
            # Soft delete the associated photo, content, and category if they exist
            if statistic.is_deleted:
                return Response({"message": "آمار از قبل حذف نرم شده است."}, status=status.HTTP_400_BAD_REQUEST)
            # if statistic.photo:
            #     statistic.photo.is_deleted = True
            #     statistic.photo.save()
            ...
            return Response({"message": "آمار و تمام وابستگی‌های آن به صورت نرم پاک شدند."}, status=status.HTTP_200_OK)

        elif delete_type == 3:
            # Hard delete: Remove the record from the database
            try:
                statistic.delete()
                return Response({"message": "آمار به صورت فیزیکی پاک شد."}, status=status.HTTP_200_OK)
            ...
            try:
                ...
                statistic.delete()
                return Response({"message": "آمار و تمام وابستگی‌های آن به صورت فیزیکی پاک شدند."},
                                status=status.HTTP_200_OK)
            except Exception as e:
                return Response({"message": f"حذف ناموفق: {str(e)}"}, status=status.HTTP_400_BAD_REQUEST)
...
class StatisticUndeleteView(APIView):

    @extend_schema(
        request=DeleteSerializer,  # The request body will contain the `id` of the cms
        responses={200: OpenApiResponse(response=MessageAndIdSerializer)},
        description="Restore a soft-deleted Statistic by ID"
    )
    @role_decorator
    def post(self, request):
        statistic_id = request.data.get('id')
        ...
        if statistic.is_deleted:
            ...
            if delete_type == 1:
                statistic.is_deleted = False
                statistic.save()
                return Response({"message": "آمار به صورت نرم بازیابی شد."}, status=status.HTTP_200_OK)
            elif delete_type == 2:
                ...
                return Response({"message": "آمار و تمام وابستگی‌های آن به صورت نرم بازیابی شدند."},
                                status=status.HTTP_200_OK)
            else:
                return Response({"message": "نوع حذف نا معتبر."}, status=status.HTTP_400_BAD_REQUEST)
        else:
            # If the cms was not soft-deleted
            return Response({"message": "آمار مورد آمار حذف نشده است."}, status=status.HTTP_400_BAD_REQUEST)
