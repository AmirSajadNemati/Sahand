...
from security.serializers import RoleSerializer, RoleGetSerializer, RoleListSerializer, RoleGetWithFeatureSerializer


class RoleAddOrUpdateView(APIView):

    @extend_schema(
        request=RoleGetSerializer,
        responses={
    ...
    def post(self, request):
        role_id = request.data.get('id', 0)

        # If role_id is 0, it's a new cms, so create it
        if role_id == 0:
            serializer = RoleGetWithFeatureSerializer(data=request.data)
            if serializer.is_valid():
                serializer.save()
                return Response({"message": "دسترسی با موفقیت ایجاد شد!", "id": serializer.instance.id},
                                status=status.HTTP_200_OK)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

        # If role_id is provided, try to update the existing cms
        try:
            role = Role.objects.get(pk=role_id)
        except Role.DoesNotExist:
            return Response({'message': 'دسترسی مورد نظر برای تغییر یافت نشد.', "id": role_id},
                            status=status.HTTP_400_BAD_REQUEST)

        # Update the existing cms
        serializer = RoleGetWithFeatureSerializer(role, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response({"message": "دسترسی با موفقیت به روزرسانی شد!", "id": role_id}, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class RoleGetListView(APIView):
    @extend_schema(
        request=ListRequestSerializer,
        responses={
            200: OpenApiResponse(response=RoleListSerializer()),
    ...
    def post(self, request):
        ...
        if sort:
            ...
            roles = roles.order_by(sort_field)
        else:
            # Apply a default ordering (e.g., by 'id' or another field)
            roles = roles.order_by('id')  # or any field you prefer

        # Pagination
        paginator = RolePagination()
        paginated_users = paginator.paginate_queryset(roles, request)

        # Serialize the paginated data
        serializer = RoleSerializer(paginated_users, many=True)
...
class RoleGetView(APIView):
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(response=RoleGetSerializer),
    ...
    def post(self, request):
        ...
        serializer = RoleGetWithFeatureSerializer(role)

        # Prepare the response structure with manual property attribute definitions for Role fields
...
class RoleDeleteView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({"message": "دسترسی یافت نشد!"}, status=status.HTTP_400_BAD_REQUEST)

        if delete_type == 1:
            # Soft delete: Set is_deleted to True
            if role.is_deleted:
                return Response({"message": "دسترسی از قبل حذف نرم شده است."}, status=status.HTTP_400_BAD_REQUEST)
            role.is_deleted = True
            role.save()
            return Response({"message": "دسترسی به صورت نرم پاک شد."}, status=status.HTTP_200_OK)
        elif delete_type == 2:
            ...
            return Response({"message": "دسترسی و تمام وابستگی‌های آن به صورت نرم پاک شدند."}, status=status.HTTP_200_OK)

        elif delete_type == 3:
            # Hard delete: Remove the record from the database
            try:
                role.delete()
                return Response({"message": "دسترسی به صورت فیزیکی پاک شد."}, status=status.HTTP_200_OK)
            ...
            try:
                ...
                role.delete()
                return Response({"message": "دسترسی و تمام وابستگی‌های آن به صورت فیزیکی پاک شدند."},
                                status=status.HTTP_200_OK)
            except Exception as e:
                return Response({"message": f"حذف ناموفق: {str(e)}"}, status=status.HTTP_400_BAD_REQUEST)
...
class RoleUndeleteView(APIView):
    ...
    def post(self, request):
        ...
        if role.is_deleted:
            ...
            if delete_type == 1:
                ...
                role.save()
                return Response({"message": "دسترسی به صورت نرم بازیابی شد."}, status=status.HTTP_200_OK)
            elif delete_type == 2:
                # Soft delete the cms itself
                role.is_deleted = False
                role.save()
                ...
                return Response({"message": "دسترسی و تمام وابستگی‌های آن به صورت نرم بازیابی شدند."},
                                status=status.HTTP_200_OK)
            else:
                return Response({"message": "نوع حذف نا معتبر."}, status=status.HTTP_400_BAD_REQUEST)
        else:
            # If the cms was not soft-deleted
            return Response({"message": "دسترسی مورد نظر حذف نشده است."}, status=status.HTTP_400_BAD_REQUEST)
