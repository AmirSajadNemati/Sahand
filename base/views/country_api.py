...
from base.serializers import CountrySerializer, CountryListSerializer
...
class CountryAddOrUpdateView(APIView):
    ...
    def post(self, request):
        country_id = request.data.get('id', 0)

        # If country_id is 0, it's a new page, so create it
        if country_id == 0:
            serializer = CountrySerializer(data=request.data)
            if serializer.is_valid():
                serializer.save()
                return Response({"message": "کشور با موفقیت ایجاد شد!", "id": serializer.instance.id},
                                status=status.HTTP_200_OK)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

        # If country_id is provided, try to update the existing page
        try:
            country = Country.objects.get(pk=country_id)
        except Country.DoesNotExist:
            return Response({'message': 'کشور مورد نظر برای تغییر یافت نشد.', "id": country_id},
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({"message": "کشور با موفقیت به روزرسانی شد!", "id": country_id},
                            status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class CountryGetListView(APIView):

    @extend_schema(
        request=ListRequestSerializer,
        responses={
            200: OpenApiResponse(response=CountryListSerializer()),
    ...
    def post(self, request):
        ...
        countrys = Country.objects.filter(query)

        # Sorting
        if sort:
            sort_field = sort if sort.startswith('-') else f'-{sort}'
            countrys = countrys.order_by(sort_field)
        else:
            # Apply a default ordering (e.g., by 'id' or another field)
            countrys = countrys.order_by('id')  # or any field you prefer

        # Pagination
        paginator = CountryPagination()
        paginated_countrys = paginator.paginate_queryset(countrys, request)

        # Serialize the paginated data
        serializer = CountrySerializer(paginated_countrys, many=True)
...
class CountryGetView(APIView):

    @extend_schema(
        request=IdSerializer,
        responses={
            200: OpenApiResponse(response=CountrySerializer),
    ...
    def post(self, request):
        ...
        # Prepare the response structure with property attributes for Country fields
...
class CountryDeleteView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({"message": "کشور یافت نشد!"}, status=status.HTTP_400_BAD_REQUEST)

        if delete_type == 1:
            # Soft delete: Set is_deleted to True
            if country.is_deleted:
                return Response({"message": "کشور از قبل حذف نرم شده است."}, status=status.HTTP_400_BAD_REQUEST)
            country.is_deleted = True
            country.save()
            return Response({"message": "کشور به صورت نرم پاک شد."}, status=status.HTTP_200_OK)
            ...
            if country.content:
                country.content.is_deleted = True
                country.content.save()

            # Soft delete the base page itself
            country.is_deleted = True
            country.save()

            return Response({"message": "کشور و وابستگی های آن صورت نرم پاک شدند."}, status=status.HTTP_200_OK)

        elif delete_type == 3:
            # Hard delete: Remove the record from the database
            try:
                country.delete()
                return Response({"message": "کشور  به صورت فیزیکی پاک شد."}, status=status.HTTP_200_OK)
            ...
            try:
                ...
                country.delete()
                return Response({"message": "کشور و تمام وابستگی‌های آن به صورت فیزیکی پاک شدند."},
                                status=status.HTTP_200_OK)
            except Exception as e:
                return Response({"message": f"حذف ناموفق: {str(e)}"}, status=status.HTTP_400_BAD_REQUEST)
...
class CountryUndeleteView(APIView):

    @extend_schema(
        request=DeleteSerializer,  # The request body will contain the `id` of the base page
        responses={200: OpenApiResponse(response=MessageAndIdSerializer)},
        description="Restore a soft-deleted Country by ID"
    )
    @role_decorator
    def post(self, request):
        country_id = request.data.get('id')
        ...
        if country.is_deleted:
            ...
            if delete_type == 1:
                ...
                country.save()
                return Response({"message": "کشور به صورت نرم بازیابی شد."}, status=status.HTTP_200_OK)
            elif delete_type == 2:
                if country.content:
                    country.content.is_deleted = False
                    country.content.save()
                ...
                return Response({"message": "کشور و وابستگی های آن به صورت نرم بازیابی شدند."},
                                status=status.HTTP_200_OK)
            else:
                return Response({"message": "نوع بازیابی نا معتبر."}, status=status.HTTP_400_BAD_REQUEST)
        else:
            # If the base page was not soft-deleted
            return Response({"message": "کشور مورد نظر حذف نشده است."}, status=status.HTTP_400_BAD_REQUEST)
