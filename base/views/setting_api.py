...
from base.serializers import SettingSerializer, SettingListSerializer
...
class SettingAddOrUpdateView(APIView):
    ...
    def post(self, request):
        setting_id = request.data.get('id', 0)

        # If setting_id is 0, it's a new page, so create it
        if setting_id == 0:
        ...
        # If setting_id is provided, try to update the existing page
        try:
            setting = Setting.objects.get(pk=setting_id)
        except Setting.DoesNotExist:
            return Response({'message': 'کشور مورد نظر برای تغییر یافت نشد.', "id": setting_id},
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({"message": "کشور با موفقیت به روزرسانی شد!", "id": setting_id},
                            status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)


class SettingPagination(PageNumberPagination):
    page_size = 10
    max_page_size = 100
...
class SettingGetListView(APIView):

    @extend_schema(
        request=ListRequestSerializer,
        responses={
            200: OpenApiResponse(response=SettingListSerializer()),
    ...
    def post(self, request):
        ...
        if sort:
            ...
            settings = settings.order_by(sort_field)
        else:
            # Apply a default ordering (e.g., by 'id' or another field)
            settings = settings.order_by('id')  # or any field you prefer

        # Pagination
        paginator = SettingPagination()
        paginated_settings = paginator.paginate_queryset(settings, request)

        # Serialize the paginated data
        serializer = SettingSerializer(paginated_settings, many=True)
...
class SettingGetView(APIView):

    @extend_schema(
        request=IdSerializer,
        responses={
            200: OpenApiResponse(response=SettingSerializer),
    ...
    def post(self, request):
        ...
        # Prepare the response structure with property attributes for Setting fields
...
class SettingDeleteView(APIView):
    ...
    def post(self, request):
        ...
        if delete_type == 1:
            ...
            if setting.content:
                setting.content.is_deleted = True
...
class SettingUndeleteView(APIView):
    ...
    def post(self, request):
        ...
        if setting.is_deleted:
            ...
            if delete_type == 1:
                ...
                if setting.content:
                    setting.content.is_deleted = False
                    setting.content.save()
