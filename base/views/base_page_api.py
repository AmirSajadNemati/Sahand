...
from base.serializers import BasePageSerializer, BasePageListSerializer
...
class BasePageAddOrUpdateView(APIView):

    @extend_schema(
        request=BasePageSerializer,
        responses={
            200: OpenApiResponse(response=MessageAndIdSerializer),
        },
        description="Add or Update BasePage"
    )
    @role_decorator
    def post(self, request):
        base_page_id = request.data.get('id', 0)

        # If base_page_id is 0, it's a new page, so create it
        if base_page_id == 0:
            serializer = BasePageSerializer(data=request.data)
        ...
        # If base_page_id is provided, try to update the existing page
        try:
            base_page = BasePage.objects.get(pk=base_page_id)
        except BasePage.DoesNotExist:
            return Response({'message': 'صفحه مورد نظر برای تغییر یافت نشد.', "id": base_page_id},
                            status=status.HTTP_400_BAD_REQUEST)

        # Update the existing page
        serializer = BasePageSerializer(base_page, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response({"message": "صفحه با موفقیت به روزرسانی شد!", "id": base_page_id},
                            status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class BasePageGetListView(APIView):

    @extend_schema(
        request=ListRequestSerializer,
        responses={
            200: OpenApiResponse(response=BasePageListSerializer()),
            400: OpenApiResponse(description='Bad Request')
        },
        description="Get List of BasePages"
    )
    @role_decorator
    def post(self, request):
        ...
        base_pages = BasePage.objects.filter(query)

        # Sorting
        if sort:
            sort_field = sort if sort.startswith('-') else f'-{sort}'
            base_pages = base_pages.order_by(sort_field)
        else:
            # Apply a default ordering (e.g., by 'id' or another field)
            base_pages = base_pages.order_by('id')  # or any field you prefer

        # Pagination
        paginator = BasePagePagination()
        paginated_base_pages = paginator.paginate_queryset(base_pages, request)

        # Serialize the paginated data
        # Serialize the paginated data
        serialized_data = BasePageSerializer(paginated_base_pages, many=True).data  # Correct serialization
...
class BasePageGetView(APIView):

    @extend_schema(
        request=IdSerializer,
        responses={
            200: OpenApiResponse(response=BasePageSerializer),
    ...
    def post(self, request):
        ...
        try:
            base_page = BasePage.objects.get(id=base_page_id)
        except BasePage.DoesNotExist:
            return Response({"message": "یافت نشد!"}, status=status.HTTP_400_BAD_REQUEST)

        serializer = BasePageSerializer(base_page)

        # Prepare the response structure with property attributes for BasePage fields
...
class BasePageDeleteView(APIView):
    ...
    @extend_schema(
        ...
        description="Delete a specific BasePage by ID"
    )
    @role_decorator
    def post(self, request):
        base_page_id = request.data.get('id')
        ...
        if delete_type == 1:
            ...
            if base_page.is_deleted:
                return Response({"message": "صفحه اصلی از قبل حذف نرم شده است."}, status=status.HTTP_400_BAD_REQUEST)
            base_page.is_deleted = True
            base_page.save()
            return Response({"message": "صفحه اصلی به صورت نرم پاک شد."}, status=status.HTTP_200_OK)
            ...
            if base_page.content:
                base_page.content.is_deleted = True
                base_page.content.save()

            # Soft delete the base page itself
            base_page.is_deleted = True
            base_page.save()

            return Response({"message": "صفحه اصلی و محتوا به صورت نرم پاک شدند."}, status=status.HTTP_200_OK)

        elif delete_type == 3:
            # Hard delete: Remove the record from the database
            try:
                base_page.delete()
                return Response({"message": "صفحه اصلی به صورت فیزیکی پاک شد."}, status=status.HTTP_200_OK)
            ...
            try:
                if base_page.content:
                    base_page.content.delete()

                # Finally, delete the base page itself
                base_page.delete()
                return Response({"message": "صفحه اصلی و تمام وابستگی‌های آن به صورت فیزیکی پاک شدند."},
                                status=status.HTTP_200_OK)
            except Exception as e:
                return Response({"message": f"حذف ناموفق: {str(e)}"}, status=status.HTTP_400_BAD_REQUEST)
...
class BasePageUndeleteView(APIView):

    @extend_schema(
        request=DeleteSerializer,  # The request body will contain the `id` of the base page
        responses={200: OpenApiResponse(response=MessageAndIdSerializer)},
        description="Restore a soft-deleted BasePage by ID"
    )
    @role_decorator
    def post(self, request):
        base_page_id = request.data.get('id')
        ...
        if base_page.is_deleted:
            ...
            if delete_type == 1:
                base_page.is_deleted = False
                base_page.save()
                return Response({"message": "صفحه اصلی به صورت نرم بازیابی شد."}, status=status.HTTP_200_OK)
            elif delete_type == 2:
                if base_page.content:
                    base_page.content.is_deleted = False
                    base_page.content.save()
                ...
                return Response({"message": "صفحه اصلی و محتوا به صورت نرم بازیابی شدند."}, status=status.HTTP_200_OK)
            else:
                return Response({"message": "نوع بازیابی نا معتبر."}, status=status.HTTP_400_BAD_REQUEST)
        else:
            # If the base page was not soft-deleted
            return Response({"message": "صفحه اصلی مورد نظر حذف نشده است."}, status=status.HTTP_400_BAD_REQUEST)
