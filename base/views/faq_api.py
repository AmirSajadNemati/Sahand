...
from base.serializers import FaqSerializer, FaqListSerializer
...
class FaqAddOrUpdateView(APIView):
    ...
    @extend_schema(
        ...
        description="Add or Update Faq"
    )
    @role_decorator
    def post(self, request):
        faq_id = request.data.get('id', 0)

        # If faq_id is 0, it's a new page, so create it
        if faq_id == 0:
            serializer = FaqSerializer(data=request.data)
            if serializer.is_valid():
                serializer.save()
                return Response({"message": "سوالات با موفقیت ایجاد شد!", "id": serializer.instance.id},
                                status=status.HTTP_200_OK)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

        # If faq_id is provided, try to update the existing page
        try:
            faq = Faq.objects.get(pk=faq_id)
        except Faq.DoesNotExist:
            return Response({'message': 'سوالات مورد نظر برای تغییر یافت نشد.', "id": faq_id},
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({"message": "سوالات با موفقیت به روزرسانی شد!", "id": faq_id},
                            status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class FaqGetListView(APIView):

    @extend_schema(
        request=ListRequestSerializer,
        responses={
            200: OpenApiResponse(response=FaqListSerializer()),
    ...
    def post(self, request):
        ...
        if sort:
            ...
            faqs = faqs.order_by(sort_field)
        else:
            # Apply a default ordering (e.g., by 'id' or another field)
            faqs = faqs.order_by('id')  # or any field you prefer

        # Pagination
        paginator = FaqPagination()
...
class FaqGetView(APIView):
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(response=FaqSerializer),
    ...
    def post(self, request):
        ...
        properties_attribute = [
            {
                "order": 1000,
                "propertyName": "Faq_category",
                "propertyType": "System.Int64",
                "enumsSelect": [],
                "isEnum": False,
                "isEnumList": False,
                "isFK": True,
                "fkUrl": "/base/FaqCategoryList/",
                "fknLevel": False,
                "fkLevelEnd": True,
                "fkParent": "",
                "fkShow": "FaqCategoryData.Title",
                "fkMultiple": False,
                "isFile": False,
                "fileTypes": [],
                "fileUrl": "",
            ...
            {
                "order": 1000,
                "propertyName": "FaqCategoryData",
                "propertyType": "StructureAndDataBase.Datas.ViewModels.Base.FaqCategoryViewModel",
                "enumsSelect": [],
                "isEnum": False,
                "isEnumList": False,
                "isFK": False,
        ...
        # Prepare the response structure with property attributes for Faq fields
...
class FaqDeleteView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({"message": "سوالات یافت نشد!"}, status=status.HTTP_400_BAD_REQUEST)

        if delete_type == 1:
            # Soft delete: Set is_deleted to True
            if faq.is_deleted:
                return Response({"message": "سوالات از قبل حذف نرم شده است."}, status=status.HTTP_400_BAD_REQUEST)
            faq.is_deleted = True
            faq.save()
            return Response({"message": "سوالات به صورت نرم پاک شد."}, status=status.HTTP_200_OK)
            ...
            return Response({"message": "سوالات و وابستگی های آن صورت نرم پاک شدند."}, status=status.HTTP_200_OK)

        elif delete_type == 3:
            # Hard delete: Remove the record from the database
            try:
                faq.delete()
                return Response({"message": "سوالات  به صورت فیزیکی پاک شد."}, status=status.HTTP_200_OK)
            ...
            try:
                ...
                faq.delete()
                return Response({"message": "سوالات و تمام وابستگی‌های آن به صورت فیزیکی پاک شدند."},
                                status=status.HTTP_200_OK)
            except Exception as e:
                return Response({"message": f"حذف ناموفق: {str(e)}"}, status=status.HTTP_400_BAD_REQUEST)
...
class FaqUndeleteView(APIView):

    @extend_schema(
        request=DeleteSerializer,  # The request body will contain the `id` of the base page
        responses={200: OpenApiResponse(response=MessageAndIdSerializer)},
        description="Restore a soft-deleted Faq by ID"
    )
    @role_decorator
    def post(self, request):
        faq_id = request.data.get('id')
        ...
        if faq.is_deleted:
            ...
            if delete_type == 1:
                ...
                faq.save()
                return Response({"message": "سوالات به صورت نرم بازیابی شد."}, status=status.HTTP_200_OK)
            elif delete_type == 2:
                if faq.content:
                    faq.content.is_deleted = False
                    faq.content.save()
                ...
                return Response({"message": "سوالات و وابستگی های آن به صورت نرم بازیابی شدند."},
                                status=status.HTTP_200_OK)
            else:
                return Response({"message": "نوع بازیابی نا معتبر."}, status=status.HTTP_400_BAD_REQUEST)
        else:
            # If the base page was not soft-deleted
            return Response({"message": "سوالات مورد نظر حذف نشده است."}, status=status.HTTP_400_BAD_REQUEST)
