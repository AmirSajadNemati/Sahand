...
from base.models import Province
from base.serializers import ProvinceSerializer, ProvinceListSerializer
...
class ProvinceAddOrUpdateView(APIView):
    ...
    @extend_schema(
        ...
        description="Add or Update Province"
    )
    @role_decorator
    def post(self, request):
        province_id = request.data.get('id', 0)

        # If province_id is 0, it's a new page, so create it
        if province_id == 0:
            serializer = ProvinceSerializer(data=request.data)
            if serializer.is_valid():
                serializer.save()
                return Response({"message": "استان با موفقیت ایجاد شد!", "id": serializer.instance.id},
                                status=status.HTTP_200_OK)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

        # If province_id is provided, try to update the existing page
        try:
            province = Province.objects.get(pk=province_id)
        except Province.DoesNotExist:
            return Response({'message': 'استان مورد نظر برای تغییر یافت نشد.', "id": province_id},
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({"message": "استان با موفقیت به روزرسانی شد!", "id": province_id},
                            status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)


class ProvincePagination(PageNumberPagination):
    page_size = 10
    max_page_size = 100
...
class ProvinceGetListView(APIView):

    @extend_schema(
        request=ListRequestSerializer,
        responses={
            200: OpenApiResponse(response=ProvinceListSerializer()),
    ...
    def post(self, request):
        ...
        provinces = Province.objects.filter(query)

        # Sorting
        if sort:
            sort_field = sort if sort.startswith('-') else f'-{sort}'
            provinces = provinces.order_by(sort_field)
        else:
            # Apply a default ordering (e.g., by 'id' or another field)
            provinces = provinces.order_by('id')  # or any field you prefer

        # Pagination
        paginator = ProvincePagination()
        paginated_provinces = paginator.paginate_queryset(provinces, request)
...
class ProvinceGetView(APIView):

    @extend_schema(
        request=IdSerializer,
        responses={
            200: OpenApiResponse(response=ProvinceSerializer),
    ...
    def post(self, request):
        ...
        # Prepare the response structure with property attributes for Province fields
...
class ProvinceDeleteView(APIView):
    ...
    @extend_schema(
        ...
        description="Delete a specific Province by ID"
    )
    @role_decorator
    def post(self, request):
        ...
        try:
            ...
            return Response({"message": "استان یافت نشد!"}, status=status.HTTP_400_BAD_REQUEST)

        if delete_type == 1:
            # Soft delete: Set is_deleted to True
            if province.is_deleted:
                return Response({"message": "استان از قبل حذف نرم شده است."}, status=status.HTTP_400_BAD_REQUEST)
            province.is_deleted = True
            province.save()
            return Response({"message": "استان به صورت نرم پاک شد."}, status=status.HTTP_200_OK)
            ...
            if province.content:
                province.content.is_deleted = True
                province.content.save()

            # Soft delete the base page itself
            province.is_deleted = True
            province.save()

            return Response({"message": "استان و وابستگی های آن صورت نرم پاک شدند."}, status=status.HTTP_200_OK)

        elif delete_type == 3:
            # Hard delete: Remove the record from the database
            try:
                province.delete()
                return Response({"message": "استان  به صورت فیزیکی پاک شد."}, status=status.HTTP_200_OK)
            ...
            try:
                if province.content:
                    province.content.delete()

                # Finally, delete the base page itself
                province.delete()
                return Response({"message": "استان و تمام وابستگی‌های آن به صورت فیزیکی پاک شدند."},
                                status=status.HTTP_200_OK)
            except Exception as e:
                return Response({"message": f"حذف ناموفق: {str(e)}"}, status=status.HTTP_400_BAD_REQUEST)
...
class ProvinceUndeleteView(APIView):

    @extend_schema(
        request=DeleteSerializer,  # The request body will contain the `id` of the base page
        responses={200: OpenApiResponse(response=MessageAndIdSerializer)},
        description="Restore a soft-deleted Province by ID"
    )
    @role_decorator
    def post(self, request):
        province_id = request.data.get('id')
        ...
        if province.is_deleted:
            ...
            if delete_type == 1:
                ...
                province.save()
                return Response({"message": "استان به صورت نرم بازیابی شد."}, status=status.HTTP_200_OK)
            elif delete_type == 2:
                if province.content:
                    province.content.is_deleted = False
                    province.content.save()
                ...
                return Response({"message": "استان و وابستگی های آن به صورت نرم بازیابی شدند."},
                                status=status.HTTP_200_OK)
            else:
                return Response({"message": "نوع بازیابی نا معتبر."}, status=status.HTTP_400_BAD_REQUEST)
        else:
            # If the base page was not soft-deleted
            return Response({"message": "استان مورد نظر حذف نشده است."}, status=status.HTTP_400_BAD_REQUEST)
